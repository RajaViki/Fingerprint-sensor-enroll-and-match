...
boolean Adafruit_Fingerprint::verifyPassword(void) {
  ...
  if ((len == 1) && (packet[0] == FINGERPRINT_ACKPACKET) && (packet[1] == FINGERPRINT_OK))
	...
	/*
  Serial.print("\nGot packet type "); Serial.print(packet[0], HEX);
...
uint8_t Adafruit_Fingerprint::setParam(uint8_t param, uint8_t value){
	uint8_t packet[] = {FINGERPRINT_SETSYSPARAM, param, value};
	writePacket(theAddress, FINGERPRINT_COMMANDPACKET, sizeof(packet)+2, packet);
    delay(2000);
    mySerial->flush();
    return 1;
...
// MODIFIED: adjusted to use the whole capacity of fpm10; 
//now searches till page 0x0671 instead of 0x00A3
uint8_t Adafruit_Fingerprint::fingerFastSearch(void) {
  fingerID = 0xFFFF;
  confidence = 0xFFFF;
  // high speed search of slot #1 starting at page 0x0000 and page #0x0671 
  uint8_t packet[] = {FINGERPRINT_HISPEEDSEARCH, 0x01, 0x00, 0x00, 0x06, 0x71};
  writePacket(theAddress, FINGERPRINT_COMMANDPACKET, sizeof(packet)+2, packet);
  uint16_t len = getReply(packet);
...
uint16_t Adafruit_Fingerprint::getReply(uint8_t packet[], uint8_t type, uint16_t timeout) {
  uint8_t reply[20], idx, val;
  uint16_t timer=0;
  uint16_t len;
...
while (true) {
    ...
    // something to read!
    val = mySerial->read();
    if ((type == ACK) || (idx <= 8)){	//typical ACK packet
    	reply[idx] = val;
	}
	else if ((type == DATA) && (idx > 8)){	// image data packet
		Serial.write(val);
		idx++;
		if (idx <= len+8){
			continue;
		}
		mySerial->read();	// ignore checksum
		mySerial->read();
		packet[0] = reply[6]; // packet type
		return len;
	}
#ifdef FINGERPRINT_DEBUG
